...
const SPRITE_RENDER_W = FRAME_W * SPRITE_SCALE;
const SPRITE_RENDER_H = FRAME_H * SPRITE_SCALE;
...
const BODY_HEIGHT_PX = SPRITE_RENDER_H;
...
function spriteFrameForState(state: GameState): number {
  switch (state) {
    case "RUNNING":
      return 0;
    case "PREPARE":
...
const GameScene = forwardRef<GameSceneHandle, GameSceneProps>(function GameScene(
  ...
  const setFig = useCallback(
    (x: number, alt: number, rot: number) => {
      ...
      if (!fig) return;
      const a = anim.current;
      const tx = (x - SPRITE_RENDER_W / 2).toFixed(1);
  ...
  const setSpriteFrame = useCallback((frame: number) => {
    const sprite = spriteRef.current;
    if (!sprite) return;
    const sx = (frame % SPRITE_COLS) * FRAME_W;
    const sy = Math.floor(frame / SPRITE_COLS) * FRAME_H;
    sprite.style.backgroundPosition = `-${sx * SPRITE_SCALE}px -${sy * SPRITE_SCALE}px`;
  }, []);
  ...
  }, [setGameState, showBanner, onHistoryPush, reset]);
  ...
  }, [setGameState, setBalance, showBanner, onHistoryPush, reset]);
  ...
  useEffect(() => {
    ...
    const tick = (time: number) => {
      ...
      if (a.state === "RUNNING") {
        ...
        if (a.frameTimer >= 100) {
          a.frameTimer = 0;
          a.spriteFrame = (a.spriteFrame + 1) % SPRITE_COLS;
        }
      } else {
        a.frameTimer = 0;
        a.spriteFrame = spriteFrameForState(a.state);
      }
      setSpriteFrame(a.spriteFrame);
  ...
  }, [drawScene, setFig, setSpriteFrame, onPnlChange, splat, getTerrainY, getTerrainSlope, getTerrainNormal, setGameState]);
    ...
    <div className="stage" ref={stageRef}>
      ...
      <div
        ...
        style={{
          zIndex: 3,
          opacity: 1,
          width: SPRITE_RENDER_W,
          height: SPRITE_RENDER_H,
      ...
      >
        ...
        <div
          ...
          style={{
            width: SPRITE_RENDER_W,
            height: SPRITE_RENDER_H,
            backgroundImage: `url(${SPRITESHEET_SRC})`,
            backgroundRepeat: "no-repeat",
            backgroundSize: `${FRAME_W * SPRITE_COLS * SPRITE_SCALE}px ${FRAME_H * 2 * SPRITE_SCALE}px`,
            backgroundPosition: `0px -${FRAME_H * SPRITE_SCALE}px`,
