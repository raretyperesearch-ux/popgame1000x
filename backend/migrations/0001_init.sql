...
create unique index if not exists trades_wallet_idx_uniq
    on public.trades (wallet_address, trade_index);

create index if not exists trades_did_opened_at_idx
    on public.trades (did, opened_at desc);

create index if not exists trades_wallet_opened_at_idx
    on public.trades (wallet_address, opened_at desc);
...
create or replace view public.trade_leaderboard as
    select
        wallet_address,
...
drop policy if exists "trades read public" on public.trades;
create policy "trades read public"
